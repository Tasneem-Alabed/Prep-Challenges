...
let key =int1;
    let arr=[];
     for(let i = 0 ; key >= -4  ;i++){
       ...
       key=key-int2;
       if(key ==-4){
        for(let x = arr.length-1 ; key <= int1  ;x++){
...
//
// Ex: <a href="http://"><h1><b>HackerRank</b></h1></a> ==> "www.hackerrank.com"
...
let result= str.indexOf('www');
let result2 = str.indexOf('>');
let result3 = str.substring(result, result2-1);
    return result3;
...
const result = str.replaceAll(' ', '').replaceAll(',','').replaceAll(':','').toLowerCase();

for(let x = 0 ;x < result.length/2; x++){
    if(result[x]!=result[result.length-x-1]){
        return false;
    }
}
return true;
